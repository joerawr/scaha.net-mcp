...
function detectScoreboardDom(html: string) {
  ...
  const teamSelect =
    selectNodes.find(
      (sel) => sel !== seasonSelect && sel !== scheduleSelect
  ...
  const dom: ScoreboardDomIds = {
    ...
    seasonField: seasonSelect?.name || seasonSelect?.id || `${formId}:j_id_4kInner`,
    scheduleSelectId: scheduleSelect?.id || `${formId}:j_id_4nInner`,
    scheduleField: scheduleSelect?.name || scheduleSelect?.id || `${formId}:j_id_4nInner`,
...
export function parseScoreboardPage(html: string): {
  ...
  const seasons = selectNodes.find((sel) => sel.id === dom.seasonSelectId)?.options || [];
  const schedules = selectNodes.find((sel) => sel.id === dom.scheduleSelectId)?.options || [];
  const teams = selectNodes.find((sel) => sel.id === dom.teamSelectId)?.options || [];
...
function detectStatsCentralDom(html: string) {
  ...
  const dom: StatsCentralDomIds = {
    ...
    scheduleField: scheduleSelect?.name || scheduleSelect?.id || `${formId}:schedulelistInner`,
